...
public class TaskM {
    public static void main(String[] args) {
        ...
        for (int request = 0; request < numOfRequests; ++request) {
            ...
            int[] difficulty = new int[numOfDays];
            ...
            for (int j = numOfDays - 1; j >= 0; --j) {
                for (int i = 0; i < j; ++i) {
                    int difficultyK = 2 * difficulty[j] - difficulty[i];
                    result += count.getOrDefault(difficultyK, 0);
                }
                count.put(difficulty[j], count.getOrDefault(difficulty[j], 0) + 1);
            }
