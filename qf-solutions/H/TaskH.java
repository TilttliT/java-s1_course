...
public class TaskH {
    public static void main(String[] args) {
        ...
        final int numOfTransaction = in.nextInt();
        int[] numOfQueries = new int[numOfTransaction];
        int maxNumOfQueries = 0;
        int sumNumOfQueries = 0;
        for (int i = 0; i < numOfTransaction; ++i) {
            numOfQueries[i] = in.nextInt();
            maxNumOfQueries = Math.max(maxNumOfQueries, numOfQueries[i]);
            sumNumOfQueries += numOfQueries[i];
        }

        int[] firstIndexTransaction = new int[sumNumOfQueries];
        for (int i = 0, j = 0, prefix = 0; i < firstIndexTransaction.length; ++i) {
            if (i - prefix == numOfQueries[j]) {
                prefix += numOfQueries[j];
                ++j;
            }
            firstIndexTransaction[i] = prefix;
        }

        int numberOfRequests = in.nextInt();
        Map<Integer, Integer> ans = new HashMap<>();
        for (int request = 0; request < numberOfRequests; ++request) {
            final int batchSize = in.nextInt();
            if (batchSize < maxNumOfQueries) {
                System.out.println("Impossible");
            } else if (!ans.containsKey(batchSize)) {
                int result = 0;
                int index = 0;
                do {
                    index = firstIndexTransaction[index] + batchSize;
                    ++result;
                } while (index < sumNumOfQueries);
                System.out.println(result);
                ans.put(batchSize, result);
            } else {
                System.out.println(ans.get(batchSize));
            }
        }
    }
}
